...
files.forEach(file => {
    ...
    // -------------------------------------------------------------
    // 1. Rename Navigation Sidebar (Mục lục)
    // -------------------------------------------------------------
    ...
    if (html.includes(navOld)) {
        ...
        console.log("- Sidebar menu items renamed successfully.");
    } else {
        // Try regex
        const navRegex = /<ul class="nav-list">[\s\S]*?<\/ul>/i;
        html = html.replace(navRegex, navNew);
        console.log("- Sidebar menu items renamed via regex.");
    }

    // -------------------------------------------------------------
    // 2. Update dynamic title JavaScript setter
    // -------------------------------------------------------------
    const jsTitlesOld = `            } else if (pageId === 'page1') {
    ...
    const jsTitlesNew = `            } else if (pageId === 'page1') {
    ...
    if (html.includes(jsTitlesOld)) {
        html = html.replace(jsTitlesOld, jsTitlesNew);
        console.log("- JS dynamic page titles updated.");
    } else {
        // Try regex-based replacement
        const jsTitlesRegex = /\} else if \(pageId === 'page1'\) \{[\s\S]*?pageTitle\.innerText = "Báo cáo 03:[\s\S]*?\}/i;
        html = html.replace(jsTitlesRegex, `} else if (pageId === 'page1') {
                pageTitle.innerText = "Báo cáo 01: Đánh giá sơ bộ Sức khỏe thương hiệu HUCE";
                pageSubtitle.innerText = "Trường Đại học Xây dựng Hà Nội (HUCE)";
            } else if (pageId === 'page2') {
                pageTitle.innerText = "Báo cáo 02: Đề xuất Hành động & Lộ trình triển khai";
        ...
        console.log("- JS dynamic page titles updated via regex.");
    }

    // -------------------------------------------------------------
    // 3. Extract the Brand Health Dashboard from old Page 1
    // -------------------------------------------------------------
    ...
    const endIdx = html.lastIndexOf('<div class="print-page-section">', targetHeaderIdx);
    
    if (startIdx === -1 || endIdx === -1) {
        console.log("- Error: Dashboard metrics extraction failed. Skipping Page 1 rebuild.");
        return;
    }
    
    const dashboardHTML = html.substring(startIdx, endIdx).trim();
    console.log("- Extracted Dashboard HTML block. Size:", dashboardHTML.length);
    
    // -------------------------------------------------------------
    // 4. Rebuild Page 1 content matching 01_danh_gia_suc_khoe_thuong_hieu.md
    // -------------------------------------------------------------
    ...
    const newPage1HTML = `<div id="page1-content" class="tab-view">
                ...
                <div class="document-vault-bar" style="margin-bottom: 1.5rem;">
                    ...
                    <a href="javascript:void(0)" onclick="scrollToSection('p1-sec1')" class="vault-btn">1. Nhu Cầu Cấp Thiết</a>
                    <a href="javascript:void(0)" onclick="scrollToSection('p1-sec2')" class="vault-btn">2. Cảnh Báo Pháp Lý</a>
                    <a href="javascript:void(0)" onclick="scrollToSection('p1-sec3')" class="vault-btn">3. Khảo Sát Thực Trạng</a>
                    <a href="javascript:void(0)" onclick="scrollToSection('p1-sec4')" class="vault-btn">4. Phân Tích SWOT</a>
                    <a href="javascript:void(0)" onclick="scrollToSection('p1-sec5')" class="vault-btn">5. Đề Xuất Giải Pháp</a>
                </div>

                <!-- SECTION 1: NHU CẦU CẤP THIẾT & ĐỊNH HƯỚNG TIẾP CẬN -->
                <div class="print-page-section"><div class="panel-card" id="p1-sec1">
                    <h3 style="text-transform: uppercase; color: var(--primary);">1. NHU CẦU CẤP THIẾT &amp; ĐỊNH HƯỚNG TIẾP CẬN</h3>
                    ...
                    <p style="font-style: italic; color: var(--text-muted);">Các hành động này nhằm giải quyết triệt để các vấn đề cốt lõi dưới đây về thương hiệu và truyền thông của nhà trường, trong đó cột mốc Đại lễ H60 đóng vai trò là động lực thúc đẩy.</p>
                </div></div>

                <!-- SECTION 2: CẢNH BÁO PHÁP LÝ KHẨN CẤP -->
                <div class="print-page-section"><div class="panel-card" id="p1-sec2">
                    <h3 style="text-transform: uppercase; color: #ef4444;">2. CẢNH BÁO PHÁP LÝ KHẨN CẤP: CHƯA ĐĂNG KÝ BẢO HỘ SỞ HỮU TRÍ TUỆ</h3>
                    ...
                    <h4 style="font-size: 0.95rem; font-weight: 700; color: var(--primary); margin: 1.2rem 0 0.5rem 0;">Các rủi ro cụ thể nếu không hành động ngay:</h4>
                    ...
                    <h4 style="font-size: 0.95rem; font-weight: 700; color: var(--primary); margin: 1.2rem 0 0.5rem 0;">Đề xuất hành động:</h4>
                    <ul style="padding-left: 1.2rem; line-height: 1.5; color: #475569;">
                ...
                <!-- SECTION 3: KHẢO SÁT THỰC TRẠNG TÀI SẢN & KÊNH TRUYỀN THÔNG HUCE -->
                <div class="print-page-section"><div class="panel-card" id="p1-sec3">
                    <h3 style="text-transform: uppercase; color: var(--primary);">3. KHẢO SÁT THỰC TRẠNG TÀI SẢN &amp; KÊNH TRUYỀN THÔNG HUCE (SỐ LIỆU ĐỊNH LƯỢNG)</h3>
                    ...
                    <h4 style="color: var(--primary); font-size: 0.95rem; font-weight: 700; margin-bottom: 0.5rem;">3.1. Sự phân mảnh tài sản thương hiệu (Brand Assets Audit)</h4>
                    ...
                    <h4 style="color: var(--primary); font-size: 0.95rem; font-weight: 700; margin-bottom: 0.5rem;">3.2. Hiệu suất các Kênh truyền thông trực tuyến</h4>
                    <ul style="padding-left: 1.2rem; line-height: 1.5; color: #475569; margin-bottom: 1.5rem;">
                    ...
                    <!-- Live indicators Dashboard restored -->
                    ${dashboardHTML}
                </div></div>

                <!-- SECTION 4: PHÂN TÍCH SWOT RÚT GỌN -->
                <div class="print-page-section"><div class="panel-card" id="p1-sec4">
                    <h3 style="text-transform: uppercase; color: var(--primary);">4. PHÂN TÍCH SWOT RÚT GỌN</h3>
                ...
                <!-- SECTION 5: ĐỀ XUẤT GIẢI PHÁP TRỌNG TÂM -->
                <div class="print-page-section"><div class="panel-card" id="p1-sec5">
                    <h3 style="text-transform: uppercase; color: var(--primary);">5. ĐỀ XUẤT GIẢI PHÁP TRỌNG TÂM</h3>
    ...
    console.log("- Page 1 content replaced with standardized 5 sections.");

    // -------------------------------------------------------------
    // 5. Update timeline dates on Page 2
    // -------------------------------------------------------------
    // GĐ 03: 16/07/2026 - 31/08/2026 -> 16/07/2026 - 15/08/2026
    html = html.replace('16/07/2026 - 31/08/2026', '16/07/2026 - 15/08/2026');
    
    // GĐ 04 title & date:
    // Quy trình Quản lý SOPs &amp; Đóng gói Quà tặng VIP (Phục vụ Đại lễ H60) -> Quy trình Quản lý SOPs &amp; Đóng gói Quà tặng VIP (Phục vụ trước Đại lễ)
    // 01/09/2026 - 15/10/2026 -> 16/08/2026 - 15/09/2026
    html = html.replace(
        'Quy trình Quản lý SOPs &amp; Đóng gói Quà tặng VIP (Phục vụ Đại lễ H60)',
        'Quy trình Quản lý SOPs &amp; Đóng gói Quà tặng VIP (Phục vụ trước Đại lễ)'
    );
    html = html.replace('01/09/2026 - 15/10/2026', '16/08/2026 - 15/09/2026');
    
    // GĐ 05: 16/10/2026 - 15/04/2027 -> 16/09/2026 - 15/11/2026
    html = html.replace('16/10/2026 - 15/04/2027', '16/09/2026 - 15/11/2026');
    
    // GĐ 06: Dài hạn sau khi bàn giao -> 16/11/2026 - 15/06/2027
    html = html.replace('Dài hạn sau khi bàn giao', '16/11/2026 - 15/06/2027');
    
    console.log("- Page 2 timeline dates updated successfully.");
    
    fs.writeFileSync(filePath, html, 'utf8');
    console.log(`=== SUCCESSFULLY REBUILT ${file} (v4) ===`);
});
