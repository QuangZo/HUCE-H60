...
// Let's restore the file first to ensure we start from a clean state
const { execSync } = require('child_process');
execSync('git restore 03_bao_gia_va_specs_chi_tiet.md');

let mdContent = fs.readFileSync(mdPath, 'utf8');

const splitToken1 = '## GÓI 1:';
...
if (idx1 !== -1 && idx2 !== -1) {
    ...
    const middleSection = `## I. GÓI TỔNG THỂ: CHUẨN HÓA HỆ THỐNG THƯƠNG HIỆU & VẬN HÀNH ĐỒNG HÀNH (800.000.000 VND)
...
`;
    ...
    console.log('Successfully updated 03_bao_gia_va_specs_chi_tiet.md using clean splits.');
} else {
    console.log('Error: split tokens not found.');
}
